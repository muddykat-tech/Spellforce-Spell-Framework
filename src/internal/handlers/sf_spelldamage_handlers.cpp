...
uint16_t invulnerability_dmg_handler(SF_CGdFigureToolbox *_this,
                                     uint16_t source, uint16_t target,
                                     uint16_t current_damage,
                                     uint16_t is_spell_damage,
                                     uint32_t is_ranged_damage,
                                               ...
                                               uint16_t spell_id)
{
    ...
    while (spell_index != 0)
    {
        toolboxAPI.removeSpellFromList(_this, target, spell_index);
        spellAPI.figTryUnfreeze(_this->CGdSpell, spell_index, target);
        tryClearCheckSpellsBeforeJob(_this->CGdSpell, spell_index, target);
        spellAPI.figClrChkSplBfrChkBattle(_this->CGdSpell, spell_index, target);
        spell_index = toolboxAPI.getSpellIndexOfType(_this, target, spell_data.spell_line_id, 0);
    }
    return current_damage;
}
