...
void __thiscall quickness_end_handler(SF_CGdSpell *_this, uint16_t spell_index)
{
    ...
    if (toolboxAPI.hasSpellOnIt(_this->SF_CGdFigureToolBox, target_index, spell_line))
    {
...
void __thiscall mutation_end_handler(SF_CGdSpell *_this, uint16_t spell_index)
{
                                ...
                                spell_data.spell_line_id))
    {
        ...
        stat_value =
            _this->SF_CGdFigure->figures[target_index].resistance_ice.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure, RESISTANCE_ICE,
                                          target_index, recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].cast_speed.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        ...
        stat_value =
            _this->SF_CGdFigure->figures[target_index].resistance_black.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure, RESISTANCE_BLACK,
                                          target_index, recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].walk_speed.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure, WALK_SPEED,
                                          target_index, recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].charisma.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure, CHARISMA,
                                          target_index, recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].dexterity.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        ...
        stat_value =
            _this->SF_CGdFigure->figures[target_index].resistance_mental.
            base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure,
                                          RESISTANCE_MENTAL, target_index,
                                          recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].strength.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
        figureAPI.addBonusMultToStatistic(_this->SF_CGdFigure, STRENGTH,
                                          target_index, recalc_value);

        stat_value =
            _this->SF_CGdFigure->figures[target_index].agility.base_val;
        recalc_value = stat_value - ((bonus * stat_value) / 100);
