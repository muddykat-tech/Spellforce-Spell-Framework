...
static std::map<OnHitPhase,
                std::list<std::pair<uint16_t,
                                    onhit_handler_ptr> > > s_onhit_handler_map;

void registerOnHitHandler(uint16_t spell_line_id, onhit_handler_ptr handler,
                          OnHitPhase phase)
{
    ...
    char buffer[256];
    sprintf(buffer, "Mod [%s] setting on hit handler for Spell line ID [%d]",
            g_current_mod->mod_id, spell_line_id);
    log_info(buffer);
