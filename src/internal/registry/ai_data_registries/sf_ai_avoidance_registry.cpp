...
static std::map<uint16_t, ai_avoidance_hander_ptr> s_ai_handler_map;

void registerAiSingleTargetHandler(uint16_t spell_line, ai_avoidance_hander_ptr handler)
{
...
ai_avoidance_hander_ptr get_single_ai_handler(uint16_t spell_line)
{
    ...
    if (it == s_ai_handler_map.end())
    {
        ...
        it = s_ai_handler_map.emplace(spell_line, &sf_ai_avoidance_default_handler).first;
    }
    return it->second;
}
