                                   ...
                                   ai_single_handler_ptr handler)
{
    ...
    {
        log_debug(DEBUG_HIGH, "%s (v%s) has replaced a single target AI Handler [%d] (Was this on purpose?)",
...
ai_single_handler_ptr get_single_ai_handler(uint16_t spell_line)
{
    ...
    if (it == s_ai_single_target_handler_map.end())
    {
        ...
        log_debug(DEBUG_HIGH, "%s Unknown spell line [%d], for ai single target handler, Assigning a default handler",
