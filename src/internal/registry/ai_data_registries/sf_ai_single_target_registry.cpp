...
static std::map<uint16_t, ai_single_handler_ptr> s_ai_avoidance_handler_map;
...
ai_single_handler_ptr get_single_ai_handler(uint16_t spell_line)
{
    ...
    if (it == s_ai_avoidance_handler_map.end())
    {
        ...
        it = s_ai_avoidance_handler_map.emplace(spell_line, &default_support_ai_handler).first;
    }
    return it->second;
}
