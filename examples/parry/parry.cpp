...
#define PARRY_LINE 0xf5
#define PARRY_JOB 0xac

// we also need to know Spell Types for vanilla spells which will be mutually exclusive with parry
// let's get them from GameData.cff and write down as hexadecimals

#define ENDURANCE_LINE 0x97 // 151
#define DURABILITY_LINE 0x9B // 155
...
// however, parry is very special, its main logic will be trigged in deal_damage handler

void __thiscall parry_damage_handler(SF_CGdFigureToolbox *_toolbox, uint16_t source, uint16_t target,
...
void __thiscall parry_effect_handler(SF_CGdSpell *_this, uint16_t spell_index)
{
    ...
    if (tick_current == 0)
        //spell start
        {
            // let's trigger refresh for parry or other vanilla spells which work in the same way
...
int __thiscall parry_endurance_durability_refresh_handler(SF_CGdSpell *_this, uint16_t spell_index) //we casted shieldwall group again before the previous expired
{
    SF_GdSpell *spell = &_this->active_spell_list[spell_index];
    ...
    if (toolboxAPI->hasSpellOnIt(_this->SF_CGdFigureToolBox, source_index, PARRY_LINE))
       // the PARRY spell already exists on the target
        {
            ...
            uint16_t pruned_spell_index = toolboxAPI->getSpellIndexOfType(_this->SF_CGdFigureToolBox, source_index, PARRY_LINE, spell_index);
    ...
    if (toolboxAPI->hasSpellOnIt(_this->SF_CGdFigureToolBox, source_index, ENDURANCE_LINE))
       // the ENDURANCE spell already exists on the target
        {
            uint16_t pruned_spell_index = toolboxAPI->getSpellIndexOfType(_this->SF_CGdFigureToolBox, source_index, ENDURANCE_LINE, spell_index);
            spellAPI->setEffectDone(_this, pruned_spell_index, 0);
        }

    if (toolboxAPI->hasSpellOnIt(_this->SF_CGdFigureToolBox, source_index, DURABILITY_LINE))
       // the DURABILITY spell already exists on the target
        {
            uint16_t pruned_spell_index = toolboxAPI->getSpellIndexOfType(_this->SF_CGdFigureToolBox, source_index, DURABILITY_LINE, spell_index);
            spellAPI->setEffectDone(_this, pruned_spell_index, 0);
        }
...
extern "C" __declspec(dllexport) void InitModule(SpellforceSpellFramework *framework)
{
    ...
    registrationAPI->linkEffectHandler(parry_spell, PARRY_JOB, &melee_group_ability_effect_handler);
    registrationAPI->linkRefreshHandler(parry_spell, &parry_endurance_durability_refresh_handler);
    registrationAPI->linkDamageHandler(damage_spell, &parry_damage_deal_handler);
    registrationAPI->linkEndHandler(parry_spell, &shield_wall_end_handler);


    // the Parry might interfere with the effect bestowed by Endurance or Durability vanilla spells
    // we're not planning

    SFSpell *endurance_spell = registrationAPI->registerSpell(ENDURANCE_LINE);
    registrationAPI->linkRefreshHandler(endurance_spell, &parry_endurance_durability_refresh_handler);

    SFSpell *durability_spell = registrationAPI->registerSpell(DURABILITY_LINE);
    registrationAPI->linkRefreshHandler(endurance_spell, &parry_endurance_durability_refresh_handler);
