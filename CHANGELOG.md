...
- Custom loading screens. `sfsf\screens.json` takes a map name and assigns a `.msb` mesh under `mesh\` to it.
...
- Structures: `SF_CGdMain`, `CGdMain_data`, `GdAvatarInternal`, `GdAvatarData`, `GdAvatar`, `AutoClass82`, `SF_GameInfo`, `CAppSession`, `CMnuScreen`, `CUtlConfigFile`.
...
- `effect_self_illusion` counts existing clones before summoning, carries over source scaling, zeroes clone weapon damage and places clones with `findClosestFreePosition`.
